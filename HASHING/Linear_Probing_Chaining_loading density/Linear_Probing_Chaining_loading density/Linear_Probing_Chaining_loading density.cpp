...
void shiftfolding(char numcode[20][30], int shift[20])
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		{
			shift[i] += 100 * (numcode[i][j] - '0') + 10 * (numcode[i][j + 1] - '0') + (numcode[i][j + 2] - '0');
		}
		...
		else if ((strlen(numcode[i]) - j) % 3 == 0)
		{
			...
			shift[i] += 10 * (numcode[i][j] - '0') + (numcode[i][j + 1] - '0');
		}
		else
		{
			shift[i] += numcode[i][j] - '0';
		}
	}
}
...
void mod10(int testarray[], int newarray[10][3])
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		newarray[resi][resicount[resi]] = testarray[i];
		resicount[resi]++;
	}
}
